...
def main():
    ...
    with httpx.Client(timeout=30, follow_redirects=True, headers={'User-Agent': 'mm-retro/1.0'}) as client:
        index = pools.build_pool_index(client, DEFAULT_LIMIT, PER_POOL_LIMITS)
    print(f'Index total: {len(index)} hash uniques')
    with psycopg.connect(host=pg_host, dbname=pg_db, user=pg_user, password=pg_password, autocommit=False) as conn:
        updated = 0
        ...
        with conn.cursor() as cur:
            cur.execute("SELECT COALESCE(miner_pool,'unknown'), COUNT(*) FROM blocks WHERE is_canonical = TRUE AND timestamp_human >= NOW() - INTERVAL '24 hours' GROUP BY 1 ORDER BY 2 DESC")
