...
- **Dashboard** — React SPA: block height, network hashrate, block-time variance, mempool history,
  block reward / emission, mining-pool distribution with **centralization metrics (largest-pool
  share and Nakamoto coefficient)**, an interactive chain-fork visualizer and reorg/orphan history.
  Each canonical block in the visualizer names its pool and how the attribution was established
  (proven / claimed / inferred), and merge-mined blocks are flagged. A live XMR price card pairs a
  centralized quote with the **Haveno street price**. Available in **English, French and Spanish**,
  light/dark themes.
...
Roughly a quarter of blocks end up `unknown`, and that number is reported as-is rather than
...
built up. Currently around 60% of blocks are merge-mined.
